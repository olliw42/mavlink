...
MAVLINK_HELPER uint8_t mavlink_parse_nextchar(mavlink_message_t* msg, mavlink_status_t* status, uint8_t c)
{
    ...
    case MAVLINK_PARSE_STATE_GOT_COMPID:
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_IN_MAVLINK1) {
            ...
            uint8_t min_message_length = msg_entry ? msg_entry->min_msg_len : 0;
            uint8_t max_message_length = msg_entry ? msg_entry->max_msg_len : 0;
            if (msg->len < min_message_length || msg->len > max_message_length) {
                _mav_parse_error(status);
                status->parse_state = MAVLINK_PARSE_STATE_IDLE;
                break;
            }
    ...
    case MAVLINK_PARSE_STATE_GOT_PAYLOAD: {
#ifdef MAVLINK_CHECK_MESSAGE_LENGTH
        const mavlink_msg_entry_t* msg_entry = msg->msg_entry;
#else
        const mavlink_msg_entry_t* msg_entry = mavlink_get_msg_entry(msg->msgid);
        msg->msg_entry = msg_entry;
#endif
