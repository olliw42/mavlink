...
//OW
/**
 * @brief Pack a utm_global_position message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_utm_global_position_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint64_t time, const uint8_t *uas_id, int32_t lat, int32_t lon, int32_t alt, int32_t relative_alt, int16_t vx, int16_t vy, int16_t vz, uint16_t h_acc, uint16_t v_acc, uint16_t vel_acc, int32_t next_lat, int32_t next_lon, int32_t next_alt, uint16_t update_rate, uint8_t flight_state, uint8_t flags)
{
...
#else
    mavlink_utm_global_position_t* packet = (mavlink_utm_global_position_t*)(&mav_txbuf[header_len]);
    packet->time = time;
    packet->lat = lat;
    packet->lon = lon;
    packet->alt = alt;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_UTM_GLOBAL_POSITION, MAVLINK_MSG_ID_UTM_GLOBAL_POSITION_MIN_LEN, MAVLINK_MSG_ID_UTM_GLOBAL_POSITION_LEN, MAVLINK_MSG_ID_UTM_GLOBAL_POSITION_CRC);
}
//OWEND
