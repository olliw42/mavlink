...
//OW
/**
 * @brief Pack a mission_current message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_mission_current_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint16_t seq)
{
    uint8_t header_len;
...
#else
    mavlink_mission_current_t* packet = (mavlink_mission_current_t*)(&mav_txbuf[header_len]);
    packet->seq = seq;

#endif

    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MISSION_CURRENT, MAVLINK_MSG_ID_MISSION_CURRENT_MIN_LEN, MAVLINK_MSG_ID_MISSION_CURRENT_LEN, MAVLINK_MSG_ID_MISSION_CURRENT_CRC);
}
//OWEND
