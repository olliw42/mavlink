...
//OW
/**
 * @brief Pack a uavionix_adsb_out_dynamic message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_uavionix_adsb_out_dynamic_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint32_t utcTime, int32_t gpsLat, int32_t gpsLon, int32_t gpsAlt, uint8_t gpsFix, uint8_t numSats, int32_t baroAltMSL, uint32_t accuracyHor, uint16_t accuracyVert, uint16_t accuracyVel, int16_t velVert, int16_t velNS, int16_t VelEW, uint8_t emergencyStatus, uint16_t state, uint16_t squawk)
{
...
#else
    mavlink_uavionix_adsb_out_dynamic_t* packet = (mavlink_uavionix_adsb_out_dynamic_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_UAVIONIX_ADSB_OUT_DYNAMIC, MAVLINK_MSG_ID_UAVIONIX_ADSB_OUT_DYNAMIC_MIN_LEN, MAVLINK_MSG_ID_UAVIONIX_ADSB_OUT_DYNAMIC_LEN, MAVLINK_MSG_ID_UAVIONIX_ADSB_OUT_DYNAMIC_CRC);
}
//OWEND
