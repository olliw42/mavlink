...
//OW
/**
 * @brief Pack a uavionix_adsb_transceiver_health_report message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_uavionix_adsb_transceiver_health_report_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t rfHealth)
{
    uint8_t header_len;
...
#else
    mavlink_uavionix_adsb_transceiver_health_report_t* packet = (mavlink_uavionix_adsb_transceiver_health_report_t*)(&mav_txbuf[header_len]);
    packet->rfHealth = rfHealth;

#endif

    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_UAVIONIX_ADSB_TRANSCEIVER_HEALTH_REPORT, MAVLINK_MSG_ID_UAVIONIX_ADSB_TRANSCEIVER_HEALTH_REPORT_MIN_LEN, MAVLINK_MSG_ID_UAVIONIX_ADSB_TRANSCEIVER_HEALTH_REPORT_LEN, MAVLINK_MSG_ID_UAVIONIX_ADSB_TRANSCEIVER_HEALTH_REPORT_CRC);
}
//OWEND
