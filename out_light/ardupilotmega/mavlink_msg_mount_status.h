...
//OW
/**
 * @brief Pack a mount_status message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_mount_status_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, int32_t pointing_a, int32_t pointing_b, int32_t pointing_c)
{
...
#else
    mavlink_mount_status_t* packet = (mavlink_mount_status_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MOUNT_STATUS, MAVLINK_MSG_ID_MOUNT_STATUS_MIN_LEN, MAVLINK_MSG_ID_MOUNT_STATUS_LEN, MAVLINK_MSG_ID_MOUNT_STATUS_CRC);
}
//OWEND
