...
//OW
/**
 * @brief Pack a remote_log_block_status message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_remote_log_block_status_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint32_t seqno, uint8_t status)
{
...
#else
    mavlink_remote_log_block_status_t* packet = (mavlink_remote_log_block_status_t*)(&mav_txbuf[header_len]);
    packet->seqno = seqno;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_REMOTE_LOG_BLOCK_STATUS, MAVLINK_MSG_ID_REMOTE_LOG_BLOCK_STATUS_MIN_LEN, MAVLINK_MSG_ID_REMOTE_LOG_BLOCK_STATUS_LEN, MAVLINK_MSG_ID_REMOTE_LOG_BLOCK_STATUS_CRC);
}
//OWEND
