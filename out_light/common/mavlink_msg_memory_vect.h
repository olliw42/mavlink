...
/**
 * @brief Pack a memory_vect message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_memory_vect_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint16_t address, uint8_t ver, uint8_t type, const int8_t *value)
{
...
#else
    mavlink_memory_vect_t* packet = (mavlink_memory_vect_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MEMORY_VECT, MAVLINK_MSG_ID_MEMORY_VECT_MIN_LEN, MAVLINK_MSG_ID_MEMORY_VECT_LEN, MAVLINK_MSG_ID_MEMORY_VECT_CRC);
}
