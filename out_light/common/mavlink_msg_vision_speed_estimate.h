...
//OW
/**
 * @brief Pack a vision_speed_estimate message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_vision_speed_estimate_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint64_t usec, float x, float y, float z, const float *covariance, uint8_t reset_counter)
{
...
#else
    mavlink_vision_speed_estimate_t* packet = (mavlink_vision_speed_estimate_t*)(&mav_txbuf[header_len]);
    packet->usec = usec;
    packet->x = x;
    packet->y = y;
    packet->z = z;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE, MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_MIN_LEN, MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_LEN, MAVLINK_MSG_ID_VISION_SPEED_ESTIMATE_CRC);
}
//OWEND
