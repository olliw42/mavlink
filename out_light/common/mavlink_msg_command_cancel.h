...
//OW
/**
 * @brief Pack a command_cancel message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_command_cancel_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint16_t command)
{
...
#else
    mavlink_command_cancel_t* packet = (mavlink_command_cancel_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_COMMAND_CANCEL, MAVLINK_MSG_ID_COMMAND_CANCEL_MIN_LEN, MAVLINK_MSG_ID_COMMAND_CANCEL_LEN, MAVLINK_MSG_ID_COMMAND_CANCEL_CRC);
}
//OWEND
