...
//OW
/**
 * @brief Pack a data_stream message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_data_stream_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t stream_id, uint16_t message_rate, uint8_t on_off)
{
...
#else
    mavlink_data_stream_t* packet = (mavlink_data_stream_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_DATA_STREAM, MAVLINK_MSG_ID_DATA_STREAM_MIN_LEN, MAVLINK_MSG_ID_DATA_STREAM_LEN, MAVLINK_MSG_ID_DATA_STREAM_CRC);
}
//OWEND
