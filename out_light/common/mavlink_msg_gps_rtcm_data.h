...
//OW
/**
 * @brief Pack a gps_rtcm_data message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_gps_rtcm_data_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t flags, uint8_t len, const uint8_t *data)
{
...
#else
    mavlink_gps_rtcm_data_t* packet = (mavlink_gps_rtcm_data_t*)(&mav_txbuf[header_len]);
    packet->flags = flags;
    packet->len = len;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_GPS_RTCM_DATA, MAVLINK_MSG_ID_GPS_RTCM_DATA_MIN_LEN, MAVLINK_MSG_ID_GPS_RTCM_DATA_LEN, MAVLINK_MSG_ID_GPS_RTCM_DATA_CRC);
}
//OWEND
