...
//OW
/**
 * @brief Pack a wifi_config_ap message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_wifi_config_ap_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   const char *ssid, const char *password, int8_t mode, int8_t response)
{
...
#else
    mavlink_wifi_config_ap_t* packet = (mavlink_wifi_config_ap_t*)(&mav_txbuf[header_len]);
    packet->mode = mode;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_WIFI_CONFIG_AP, MAVLINK_MSG_ID_WIFI_CONFIG_AP_MIN_LEN, MAVLINK_MSG_ID_WIFI_CONFIG_AP_LEN, MAVLINK_MSG_ID_WIFI_CONFIG_AP_CRC);
}
//OWEND
