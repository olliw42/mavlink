...
//OW
/**
 * @brief Pack a log_request_list message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_log_request_list_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint16_t start, uint16_t end)
{
...
#else
    mavlink_log_request_list_t* packet = (mavlink_log_request_list_t*)(&mav_txbuf[header_len]);
    packet->start = start;
    packet->end = end;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_LOG_REQUEST_LIST, MAVLINK_MSG_ID_LOG_REQUEST_LIST_MIN_LEN, MAVLINK_MSG_ID_LOG_REQUEST_LIST_LEN, MAVLINK_MSG_ID_LOG_REQUEST_LIST_CRC);
}
//OWEND
