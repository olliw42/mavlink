...
/**
 * @brief Pack a log_data message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_log_data_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint16_t id, uint32_t ofs, uint8_t count, const uint8_t *data)
{
...
#else
    mavlink_log_data_t* packet = (mavlink_log_data_t*)(&mav_txbuf[header_len]);
    packet->ofs = ofs;
    packet->id = id;
    packet->count = count;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_LOG_DATA, MAVLINK_MSG_ID_LOG_DATA_MIN_LEN, MAVLINK_MSG_ID_LOG_DATA_LEN, MAVLINK_MSG_ID_LOG_DATA_CRC);
}
