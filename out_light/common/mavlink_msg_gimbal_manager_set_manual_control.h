...
//OW
/**
 * @brief Pack a gimbal_manager_set_manual_control message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_gimbal_manager_set_manual_control_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint32_t flags, uint8_t gimbal_device_id, float pitch, float yaw, float pitch_rate, float yaw_rate)
{
...
#else
    mavlink_gimbal_manager_set_manual_control_t* packet = (mavlink_gimbal_manager_set_manual_control_t*)(&mav_txbuf[header_len]);
    packet->flags = flags;
    packet->pitch = pitch;
    packet->yaw = yaw;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_GIMBAL_MANAGER_SET_MANUAL_CONTROL, MAVLINK_MSG_ID_GIMBAL_MANAGER_SET_MANUAL_CONTROL_MIN_LEN, MAVLINK_MSG_ID_GIMBAL_MANAGER_SET_MANUAL_CONTROL_LEN, MAVLINK_MSG_ID_GIMBAL_MANAGER_SET_MANUAL_CONTROL_CRC);
}
//OWEND
