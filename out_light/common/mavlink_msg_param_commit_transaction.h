...
//OW
/**
 * @brief Pack a param_commit_transaction message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_param_commit_transaction_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint8_t response)
{
...
#else
    mavlink_param_commit_transaction_t* packet = (mavlink_param_commit_transaction_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_PARAM_COMMIT_TRANSACTION, MAVLINK_MSG_ID_PARAM_COMMIT_TRANSACTION_MIN_LEN, MAVLINK_MSG_ID_PARAM_COMMIT_TRANSACTION_LEN, MAVLINK_MSG_ID_PARAM_COMMIT_TRANSACTION_CRC);
}
//OWEND
