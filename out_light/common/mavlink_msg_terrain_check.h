...
/**
 * @brief Pack a terrain_check message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_terrain_check_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   int32_t lat, int32_t lon)
{
...
#else
    mavlink_terrain_check_t* packet = (mavlink_terrain_check_t*)(&mav_txbuf[header_len]);
    packet->lat = lat;
    packet->lon = lon;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_TERRAIN_CHECK, MAVLINK_MSG_ID_TERRAIN_CHECK_MIN_LEN, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN, MAVLINK_MSG_ID_TERRAIN_CHECK_CRC);
}
