...
//OW
/**
 * @brief Pack a mount_orientation message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_mount_orientation_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint32_t time_boot_ms, float roll, float pitch, float yaw, float yaw_absolute)
{
...
#else
    mavlink_mount_orientation_t* packet = (mavlink_mount_orientation_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MOUNT_ORIENTATION, MAVLINK_MSG_ID_MOUNT_ORIENTATION_MIN_LEN, MAVLINK_MSG_ID_MOUNT_ORIENTATION_LEN, MAVLINK_MSG_ID_MOUNT_ORIENTATION_CRC);
}
//OWEND
