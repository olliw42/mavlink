...
//OW
/**
 * @brief Pack a mission_item_reached message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_mission_item_reached_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint16_t seq)
{
    uint8_t header_len;
...
#else
    mavlink_mission_item_reached_t* packet = (mavlink_mission_item_reached_t*)(&mav_txbuf[header_len]);
    packet->seq = seq;

#endif

    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MISSION_ITEM_REACHED, MAVLINK_MSG_ID_MISSION_ITEM_REACHED_MIN_LEN, MAVLINK_MSG_ID_MISSION_ITEM_REACHED_LEN, MAVLINK_MSG_ID_MISSION_ITEM_REACHED_CRC);
}
//OWEND
