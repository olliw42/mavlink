...
//OW
/**
 * @brief Pack a cellular_status message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_cellular_status_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t status, uint8_t failure_reason, uint8_t type, uint8_t quality, uint16_t mcc, uint16_t mnc, uint16_t lac)
{
...
#else
    mavlink_cellular_status_t* packet = (mavlink_cellular_status_t*)(&mav_txbuf[header_len]);
    packet->mcc = mcc;
    packet->mnc = mnc;
    packet->lac = lac;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_CELLULAR_STATUS, MAVLINK_MSG_ID_CELLULAR_STATUS_MIN_LEN, MAVLINK_MSG_ID_CELLULAR_STATUS_LEN, MAVLINK_MSG_ID_CELLULAR_STATUS_CRC);
}
//OWEND
