...
//OW
/**
 * @brief Pack a gimbal_device_set_attitude message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_gimbal_device_set_attitude_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint16_t flags, const float *q, float angular_velocity_x, float angular_velocity_y, float angular_velocity_z)
{
...
#else
    mavlink_gimbal_device_set_attitude_t* packet = (mavlink_gimbal_device_set_attitude_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_GIMBAL_DEVICE_SET_ATTITUDE, MAVLINK_MSG_ID_GIMBAL_DEVICE_SET_ATTITUDE_MIN_LEN, MAVLINK_MSG_ID_GIMBAL_DEVICE_SET_ATTITUDE_LEN, MAVLINK_MSG_ID_GIMBAL_DEVICE_SET_ATTITUDE_CRC);
}
//OWEND
