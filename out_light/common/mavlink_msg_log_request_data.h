...
//OW
/**
 * @brief Pack a log_request_data message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_log_request_data_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint16_t id, uint32_t ofs, uint32_t count)
{
...
#else
    mavlink_log_request_data_t* packet = (mavlink_log_request_data_t*)(&mav_txbuf[header_len]);
    packet->ofs = ofs;
    packet->count = count;
    packet->id = id;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_LOG_REQUEST_DATA, MAVLINK_MSG_ID_LOG_REQUEST_DATA_MIN_LEN, MAVLINK_MSG_ID_LOG_REQUEST_DATA_LEN, MAVLINK_MSG_ID_LOG_REQUEST_DATA_CRC);
}
//OWEND
