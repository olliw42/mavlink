...
/**
 * @brief Pack a gopro_get_response message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_gopro_get_response_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t cmd_id, uint8_t status, const uint8_t *value)
{
...
#else
    mavlink_gopro_get_response_t* packet = (mavlink_gopro_get_response_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_GOPRO_GET_RESPONSE, MAVLINK_MSG_ID_GOPRO_GET_RESPONSE_MIN_LEN, MAVLINK_MSG_ID_GOPRO_GET_RESPONSE_LEN, MAVLINK_MSG_ID_GOPRO_GET_RESPONSE_CRC);
}
