...
/**
 * @brief Pack a debug_float_array message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_debug_float_array_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint64_t time_usec, const char *name, uint16_t array_id, const float *data)
{
...
#else
    mavlink_debug_float_array_t* packet = (mavlink_debug_float_array_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_DEBUG_FLOAT_ARRAY, MAVLINK_MSG_ID_DEBUG_FLOAT_ARRAY_MIN_LEN, MAVLINK_MSG_ID_DEBUG_FLOAT_ARRAY_LEN, MAVLINK_MSG_ID_DEBUG_FLOAT_ARRAY_CRC);
}
