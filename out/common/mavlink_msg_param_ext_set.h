...
/**
 * @brief Pack a param_ext_set message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_param_ext_set_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, const char *param_id, const char *param_value, uint8_t param_type)
{
...
#else
    mavlink_param_ext_set_t* packet = (mavlink_param_ext_set_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_PARAM_EXT_SET, MAVLINK_MSG_ID_PARAM_EXT_SET_MIN_LEN, MAVLINK_MSG_ID_PARAM_EXT_SET_LEN, MAVLINK_MSG_ID_PARAM_EXT_SET_CRC);
}
