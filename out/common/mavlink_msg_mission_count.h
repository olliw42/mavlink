...
/**
 * @brief Pack a mission_count message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_mission_count_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint16_t count, uint8_t mission_type)
{
...
#else
    mavlink_mission_count_t* packet = (mavlink_mission_count_t*)(&mav_txbuf[header_len]);
    packet->count = count;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MISSION_COUNT, MAVLINK_MSG_ID_MISSION_COUNT_MIN_LEN, MAVLINK_MSG_ID_MISSION_COUNT_LEN, MAVLINK_MSG_ID_MISSION_COUNT_CRC);
}
