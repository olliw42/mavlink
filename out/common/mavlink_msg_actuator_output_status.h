...
/**
 * @brief Pack a actuator_output_status message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_actuator_output_status_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint64_t time_usec, uint32_t active, const float *actuator)
{
...
#else
    mavlink_actuator_output_status_t* packet = (mavlink_actuator_output_status_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS, MAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS_MIN_LEN, MAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS_LEN, MAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS_CRC);
}
