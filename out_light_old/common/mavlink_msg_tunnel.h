...
/**
 * @brief Pack a tunnel message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_tunnel_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint16_t payload_type, uint8_t payload_length, const uint8_t *payload)
{
...
#else
    mavlink_tunnel_t* packet = (mavlink_tunnel_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_TUNNEL, MAVLINK_MSG_ID_TUNNEL_MIN_LEN, MAVLINK_MSG_ID_TUNNEL_LEN, MAVLINK_MSG_ID_TUNNEL_CRC);
}
