...
/**
 * @brief Pack a wheel_distance message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_wheel_distance_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint64_t time_usec, uint8_t count, const double *distance)
{
...
#else
    mavlink_wheel_distance_t* packet = (mavlink_wheel_distance_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_WHEEL_DISTANCE, MAVLINK_MSG_ID_WHEEL_DISTANCE_MIN_LEN, MAVLINK_MSG_ID_WHEEL_DISTANCE_LEN, MAVLINK_MSG_ID_WHEEL_DISTANCE_CRC);
}
