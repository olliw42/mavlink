...
/**
 * @brief Pack a gopro_heartbeat message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_gopro_heartbeat_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t status, uint8_t capture_mode, uint8_t flags)
{
...
#else
    mavlink_gopro_heartbeat_t* packet = (mavlink_gopro_heartbeat_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_GOPRO_HEARTBEAT, MAVLINK_MSG_ID_GOPRO_HEARTBEAT_MIN_LEN, MAVLINK_MSG_ID_GOPRO_HEARTBEAT_LEN, MAVLINK_MSG_ID_GOPRO_HEARTBEAT_CRC);
}
