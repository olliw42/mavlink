...
/**
 * @brief Pack a gopro_get_request message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_gopro_get_request_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint8_t cmd_id)
{
...
#else
    mavlink_gopro_get_request_t* packet = (mavlink_gopro_get_request_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_GOPRO_GET_REQUEST, MAVLINK_MSG_ID_GOPRO_GET_REQUEST_MIN_LEN, MAVLINK_MSG_ID_GOPRO_GET_REQUEST_LEN, MAVLINK_MSG_ID_GOPRO_GET_REQUEST_CRC);
}
