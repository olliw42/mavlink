...
/**
 * @brief Pack a mission_ack message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_mission_ack_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t target_component, uint8_t type, uint8_t mission_type)
{
...
#else
    mavlink_mission_ack_t* packet = (mavlink_mission_ack_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_MISSION_ACK, MAVLINK_MSG_ID_MISSION_ACK_MIN_LEN, MAVLINK_MSG_ID_MISSION_ACK_LEN, MAVLINK_MSG_ID_MISSION_ACK_CRC);
}
