...
/**
 * @brief Pack a timesync message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_timesync_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   int64_t tc1, int64_t ts1)
{
...
#else
    mavlink_timesync_t* packet = (mavlink_timesync_t*)(&mav_txbuf[header_len]);
    packet->tc1 = tc1;
    packet->ts1 = ts1;
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_TIMESYNC, MAVLINK_MSG_ID_TIMESYNC_MIN_LEN, MAVLINK_MSG_ID_TIMESYNC_LEN, MAVLINK_MSG_ID_TIMESYNC_CRC);
}
