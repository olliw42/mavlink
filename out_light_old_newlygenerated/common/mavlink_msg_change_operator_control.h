...
/**
 * @brief Pack a change_operator_control message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_change_operator_control_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint8_t target_system, uint8_t control_request, uint8_t version, const char *passkey)
{
...
#else
    mavlink_change_operator_control_t* packet = (mavlink_change_operator_control_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL, MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL_MIN_LEN, MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL_LEN, MAVLINK_MSG_ID_CHANGE_OPERATOR_CONTROL_CRC);
}
