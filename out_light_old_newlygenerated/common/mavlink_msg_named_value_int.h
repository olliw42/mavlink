...
/**
 * @brief Pack a named_value_int message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_named_value_int_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint32_t time_boot_ms, const char *name, int32_t value)
{
...
#else
    mavlink_named_value_int_t* packet = (mavlink_named_value_int_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_NAMED_VALUE_INT, MAVLINK_MSG_ID_NAMED_VALUE_INT_MIN_LEN, MAVLINK_MSG_ID_NAMED_VALUE_INT_LEN, MAVLINK_MSG_ID_NAMED_VALUE_INT_CRC);
}
