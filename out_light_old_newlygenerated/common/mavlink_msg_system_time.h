...
/**
 * @brief Pack a system_time message into a transmit buffer
 ...
 */
static inline uint16_t mavlink_msg_system_time_pack_txbuf(char* mav_txbuf, mavlink_status_t* mav_status, uint8_t system_id, uint8_t component_id,
                                   uint64_t time_unix_usec, uint32_t time_boot_ms)
{
...
#else
    mavlink_system_time_t* packet = (mavlink_system_time_t*)(&mav_txbuf[header_len]);
    ...
    return mavlink_finalize_message_txbuf(mav_txbuf, mav_status, system_id, component_id,
                                          MAVLINK_MSG_ID_SYSTEM_TIME, MAVLINK_MSG_ID_SYSTEM_TIME_MIN_LEN, MAVLINK_MSG_ID_SYSTEM_TIME_LEN, MAVLINK_MSG_ID_SYSTEM_TIME_CRC);
}
